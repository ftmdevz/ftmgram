...
class OnError:
    def on_error(
        ...
        :obj:`~ftmgram.handlers.ErrorHandler`.
        ...
        def decorator(func: Callable) -> Callable:
            if isinstance(self, ftmgram.Client):
                self.add_handler(ftmgram.handlers.ErrorHandler(func, exceptions, filters), group)
            else:
                if not hasattr(func, "handlers"):
                    func.handlers = []

                func.handlers.append(
                    (ftmgram.handlers.ErrorHandler(func, exceptions, filters), group)
                )
