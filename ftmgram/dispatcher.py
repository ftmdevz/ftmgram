...
class Dispatcher:
    ...
    def __init__(self, client: "ftmgram.Client"):
        ...
        async def poll_parser(update, users, chats):
            return (
                await ftmgram.types.Poll._parse_update(self.client, update, users, chats),
                PollHandler
            )
        ...
        async def story_parser(update, users, chats):
            return (
                await ftmgram.types.Story._parse(self.client, update.story, update.peer, users, chats),
                StoryHandler
            )
        ...
        async def chat_boost_parser(update, users, chats):
            return (
                ftmgram.types.ChatBoostUpdated._parse(self.client, update, users, chats),
                ChatBoostHandler
            )

        async def purchased_paid_media_parser(update, users, chats):
            return (
                ftmgram.types.PurchasedPaidMedia._parse(self.client, update, users),
                PurchasedPaidMediaHandler
            )

        async def business_connection_parser(update, users, chats):
            return (
                ftmgram.types.BusinessConnection._parse(self.client, update, users),
                BusinessConnectionHandler
            )
        ...
        async def managed_bot_parser(update, users, chats):
            return (
                await ftmgram.types.ManagedBotUpdated._parse(self.client, update, users),
                ManagedBotUpdatedHandler
            )
        ...
        async def guest_message_parser(update, users, chats):
            ...
            for ref in update.reference_messages or []:
                await ftmgram.types.Message._parse(self.client, ref, users, chats)
            parsed, _ = await message_parser(update, users, chats)
    ...
    async def handle_update_handler_exception(
        ...
        update: "ftmgram.raw.base.Update",
        users: Dict[int, "ftmgram.raw.base.User"],
        chats: Dict[int, "ftmgram.raw.base.Chat"]
    ) -> None:
        handled = False
        try:
            for group in self.groups.values():
