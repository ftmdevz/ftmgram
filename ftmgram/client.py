...
import ftmgram
from ftmgram import __license__, __version__, enums, raw, utils
...
from ftmgram.qrlogin import QRLogin
from ftmgram.session import Auth, Session
from ftmgram.storage import SQLiteStorage, Storage
from ftmgram.types import LinkPreviewOptions, TermsOfService, User
from ftmgram.utils import ainput
...
class Client(Methods):
    ...
    TME_PUBLIC_LINK_RE = re.compile(r"^(?:https?://)?(?:www\.|(?P<subdomain>[\w-]+)\.)?(?:t(?:elegram)?\.(?:org|me|dog))/?(?P<slug>[\w-]+)?$")
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.ftmgram.org/start/auth")
    ...
    async def handle_download(self, packet):
        ...
        try:
            ...
            if isinstance(e, ftmgram.StopTransmission):
                return None
